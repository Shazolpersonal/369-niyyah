...
## 2026-05-21 - Adding accessibility attributes to Progress Rings
**Learning:** Progress indicator components (like `JourneyProgressRing`) that convey state visually should be wrapped in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue`, and a localized `accessibilityLabel` to ensure screen readers announce the progress.
**Action:** Always include `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue`, and a descriptive `accessibilityLabel` on custom progress components. Additionally, hide decorative inner text nodes with `importantForAccessibility="no"` and `accessibilityElementsHidden={true}`.
