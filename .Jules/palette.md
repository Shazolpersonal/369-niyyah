...
## 2026-06-11 - Adding accessibility attributes to JourneyProgressRing
**Learning:** For visual progress components (like circular progress rings) implemented as an SVG + Text, wrap the component in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel`. Ensure inner text nodes have `importantForAccessibility="no"` to prevent redundant reading by screen readers. Furthermore, note that the custom `translate` function (`t`) from `contexts/LanguageContext` expects double-brace syntax `{{paramName}}` for parameter interpolation, so using manual `.replace()` may be necessary if translation strings use single braces.
**Action:** Always apply standard `accessibilityRole="progressbar"` and  attributes on circular/linear progress trackers to inform users exactly where they are in a process.
## 2026-06-11 - Adding accessibility attributes to JourneyProgressRing
**Learning:** For visual progress components (like circular progress rings) implemented as an SVG + Text, wrap the component in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel`. Ensure inner text nodes have `importantForAccessibility="no"` to prevent redundant reading by screen readers. Furthermore, note that the custom `translate` function (`t`) from `contexts/LanguageContext` expects double-brace syntax `{{paramName}}` for parameter interpolation, so using manual `.replace()` may be necessary if translation strings use single braces.
**Action:** Always apply standard `accessibilityRole="progressbar"` and `accessibilityValue` attributes on circular/linear progress trackers to inform users exactly where they are in a process.
