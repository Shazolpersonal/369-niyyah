...
## 2026-05-21 - Adding accessibility attributes to Progress Rings and Toasts
**Learning:** In React Native, visual progress components (like circular rings) must be wrapped in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel`. Inner text nodes must be explicitly hidden from screen readers using `importantForAccessibility="no"` (Android) and `accessibilityElementsHidden={true}` (iOS) to prevent redundant reading. Additionally, ephemeral notifications like Toasts require `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` to ensure they are announced without interrupting the user.
**Action:** Always add proper `accessibilityRole`, `accessibilityValue`, and `accessibilityLabel` to progress indicators, and explicitly hide their child text elements. For Toasts and alerts, always use `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"`.
