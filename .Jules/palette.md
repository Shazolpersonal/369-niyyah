...
**Learning:** In React Native, ephemeral notifications like Toasts do not automatically trigger screen reader announcements. Using standard web practices doesn't apply directly here.
**Action:** For ephemeral notifications like Toasts, use `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` on the container to ensure screen readers announce the message automatically without interrupting the user.
