...
## 2025-05-19 - Adding accessibility attributes to CalendarDay
**Learning:** Custom UI components that convey status purely through visual state (like background colors in a calendar day) lack necessary context for screen readers. It's necessary to combine the core text (like the day number) with a translated status in the `accessibilityLabel`. For interactive elements rendered conditionally, the non-interactive fallback container must specify `accessible={true}` and the identical `accessibilityLabel` so screen readers accurately announce the state.
**Action:** Always combine textual info and translated visual states in `accessibilityLabel` for visual-only status components. Apply `accessible={true}` and `accessibilityLabel` to non-interactive wrappers if conditional interactivity is present, and set `importantForAccessibility="no"` to inner text components to prevent duplicate announcements.
