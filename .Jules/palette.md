...
## 2025-05-19 - Adding accessibility attributes to Progress Rings and Counters
**Learning:** In React Native, visual progress components like circular progress rings (e.g., `JourneyProgressRing` or `RepetitionCounter`) consist of custom SVG paths and text nodes that screen readers interpret as a collection of disjointed strings or simply ignore. They must be explicitly typed and described for accessibility.
**Action:** When building custom progress visualizations, ensure the outermost container has `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel`. Crucially, apply `importantForAccessibility="no"` and `accessibilityElementsHidden={true}` to any inner `<Text>` components to prevent screen readers from redundantly announcing the bare numbers.
