...
## 2026-05-19 - Adding accessibility attributes to visually complex state-dependent components
**Learning:** For custom UI components that represent complex information purely through visual state (e.g., color-coded Calendar Days indicating 'complete', 'missed', etc.), standard screen readers will only announce the child text (e.g., "15"). This misses crucial context conveyed visually by background colors.
**Action:** Provide explicit, comprehensive `accessibilityLabel` attributes that combine the core text (the date) with its visual status (the meaning of its color/state) translated appropriately using `t()`. Also, if an element may or may not be interactive (like a calendar day missing an `onPress` in certain contexts), ensure the fallback view explicitly specifies `accessible={true}` to guarantee the label is announced.
