...
## 2026-07-05 - Adding accessibility attributes to Progress Rings
**Learning:** Visual progress components (like circular progress rings) are invisible to screen readers without specific roles and values. They require `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel`.
**Action:** Always wrap visual progress indicators in an accessible container and hide inner decorative text using `importantForAccessibility="no"` (Android) and `accessibilityElementsHidden={true}` (iOS) to prevent redundant or confusing screen reader announcements.
