...
## 2026-05-21 - Adding accessibility attributes to Toasts
**Learning:** In React Native, ephemeral UI elements like Toast notifications may appear visually but remain hidden to screen readers if not properly marked.
**Action:** Always ensure Toast or snackbar containers include `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` so screen readers will announce their content automatically as they mount without interrupting current user tasks.
