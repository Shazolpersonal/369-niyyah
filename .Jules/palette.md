...
## 2026-05-02 - TextInput Accessibility Label Requirement
**Learning:** In React Native, the `placeholder` property on a `TextInput` is often insufficient for proper screen reader support. It is critical to explicitly provide an `accessibilityLabel` for inputs that don't have an associated visual label component.
**Action:** Always add an explicit `accessibilityLabel` to `TextInput` components, even if a `placeholder` is provided, especially in forms or input screens.
