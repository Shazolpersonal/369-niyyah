...
## 2026-05-21 - Adding accessibility attributes to Toast Notifications
**Learning:** In React Native, ephemeral UI elements like Toast notifications often appear and disappear without user interaction. Screen readers may ignore them entirely if they are just standard `View` components.
**Action:** When creating ephemeral, auto-dismissing notification banners or Toasts, explicitly set `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` on the container `View`. This ensures screen readers announce the contents correctly when the element mounts, without unnecessarily interrupting the user's current flow.
