...
## 2023-10-27 - Added ARIA attributes to TaskCard and Toast
**Learning:** In React Native, applying `accessibilityLabel` to interactive components (like `Pressable` or `View`) that already contain descriptive child `<Text>` elements without setting `importantForAccessibility="no"` (Android) or `accessibilityElementsHidden={true}` (iOS) on those children can cause screen readers to read both the parent label and the child text, leading to redundant and confusing announcements. Also, for ephemeral notifications like Toasts, explicitly setting `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` ensures they are automatically announced by screen readers without interrupting the user.
**Action:** When adding `accessibilityLabel` to a container that groups multiple text elements (like a card or button), remember to set `importantForAccessibility="no"` on the child `<Text>` components to prevent duplicate reading. Always use `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` for non-interactive popups or toasts that appear dynamically.
