...
## 2024-06-17 - Visual Progress Components Accessibility Redundancy
**Learning:** For visual progress components (like circular progress rings), screen readers may read child text nodes redundantly if the parent has an accessibilityLabel, or read the visual numbers without context if it doesn't.
**Action:** Wrap the component in a container with accessible={true}, accessibilityRole="progressbar", accessibilityValue={{ min, max, now }}, and a localized accessibilityLabel. Ensure inner text nodes have importantForAccessibility="no" (Android) and accessibilityElementsHidden={true} (iOS) to prevent redundant reading.
