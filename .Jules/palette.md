...
## 2026-05-23 - Adding accessibility attributes to Toast Notifications
**Learning:** In React Native, ephemeral notifications like Toasts need special accessibility handling to ensure screen readers announce them automatically without interrupting the user's flow. Standard text or view elements are often ignored when they appear dynamically.
**Action:** Always add `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` to the outermost container of custom Toast or alert components.
