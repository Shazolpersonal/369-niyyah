...
## 2026-05-21 - Adding accessibility to Progress Rings

**Learning:** Visual progress components like \`JourneyProgressRing\` and \`RepetitionCounter\` present their value visually but fail to announce it to screen readers. They need explicit accessibility properties to function correctly for visually impaired users.
**Action:** When creating visual progress indicators, ensure the wrapper component has \`accessible={true}\`, \`accessibilityRole="progressbar"\`, \`accessibilityValue={{ min, max, now }}\`, and a descriptive \`accessibilityLabel\`. Additionally, apply \`importantForAccessibility="no"\` and \`accessibilityElementsHidden={true}\` to any inner decorative \`Text\` components to prevent redundant and out-of-context reading.
