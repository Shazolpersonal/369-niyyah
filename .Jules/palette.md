...
## 2026-06-28 - Adding accessibility attributes to visual progress rings
**Learning:** In React Native, for visual progress components (like circular progress rings), wrapping the component in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel` is necessary for screen readers. Inner text nodes should have `importantForAccessibility="no"` (Android) and `accessibilityElementsHidden={true}` (iOS) to prevent redundant reading.
**Action:** Remember to explicitly define React Native accessibility props on all custom progress components and hide decorative/redundant text children.
