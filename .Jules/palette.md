...
## 2026-05-20 - Adding Accessibility to Ephemeral Toast Notifications and Visual Progress Rings
**Learning:** In React Native, custom ephemeral notifications (like Toasts) or purely visual progress indicators (like `RepetitionCounter`) are often completely ignored or poorly announced by screen readers. For Toasts, adding `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` forces the screen reader to naturally announce the content when it appears. For custom SVG progress components, it's vital to wrap them in an accessible container with an explicit translated `accessibilityLabel` (e.g., "5 of 10 completed") while hiding the inner text components (`importantForAccessibility="no"`) to prevent messy, disjointed readouts like "5 slash 10".
**Action:** Always add `accessibilityRole="alert"` to Toast containers. Always use `accessible={true}` with an overarching `accessibilityLabel` on visual data widgets while hiding their raw text descendants.
