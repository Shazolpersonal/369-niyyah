...
## 2026-05-21 - Adding accessibility attributes to Ephemeral Toasts
**Learning:** In React Native, ephemeral notifications like Toasts rendered via absolute positioning or Reanimated are often missed by screen readers because they appear dynamically outside the regular view flow. Relying solely on visual changes is insufficient for visually impaired users.
**Action:** Always apply `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` to the container of Toast or Snackbar components to ensure screen readers automatically announce their message without interrupting the user's current context.
