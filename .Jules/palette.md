...
## 2026-05-26 - Add proper accessibility traits to Toast
**Learning:** In React Native, for ephemeral notifications like Toasts, use `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` on the container to ensure screen readers announce the message automatically without interrupting the user.
**Action:** When building custom Toast notifications or auto-hiding feedback components, explicitly assign `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` to the outermost wrapper.
