...
## 2025-05-19 - Adding accessibility attributes to Progress Components
**Learning:** React Native's purely visual progress components (like SVGs or animated rings) do not implicitly convey their status to screen readers. For components like `JourneyProgressRing` and `RepetitionCounter`, it is crucial to explicitly define `accessible={true}`, `accessibilityRole="progressbar"`, and use dynamic `accessibilityValue` and localized `accessibilityLabel` props on their wrapper elements so users with visual impairments can understand the current progress state.
**Action:** Always wrap visual progress rings and gauges in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a descriptive `accessibilityLabel`. Ensure any inner text nodes used for visual display are explicitly hidden from screen readers using `importantForAccessibility="no"` and `accessibilityElementsHidden={true}` to prevent redundant announcements.
