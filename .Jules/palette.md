...
## 2026-05-06 - Adding accessibility attributes to TextInput and Forms

**Learning:** React Native's `TextInput` components do not automatically read their `placeholder` props correctly on all screen readers. Additionally, main CTA buttons or secondary actions on form screens often lack explicit labels.
**Action:** Always provide an explicit `accessibilityLabel` on `TextInput` components, even if a placeholder is present, using the existing i18n functions. Ensure form submit buttons explicitly define `accessibilityRole="button"`, `accessibilityLabel`, and dynamic `accessibilityState={{ disabled: boolean }}` when validation requires them to be disabled.
