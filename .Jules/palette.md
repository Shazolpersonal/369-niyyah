...
## 2026-07-06 - Adding accessibility attributes to Circular Progress Components
**Learning:** Visual circular progress components (like `RepetitionCounter` and `JourneyProgressRing`) lack semantic meaning for screen readers. They must be explicitly marked with `accessibilityRole="progressbar"` and `accessibilityValue={{ min, max, now }}`. Furthermore, wrapping elements should declare an `accessibilityLabel` using the translation function (`t()`), and the inner visual text nodes should be hidden from screen readers using `importantForAccessibility="no"` and `accessibilityElementsHidden={true}` to prevent redundant announcements.
**Action:** Whenever creating a custom visual indicator of progress (such as an SVG ring), always ensure the container defines the correct progress bar accessibility attributes and inner text labels are silenced from the screen reader to maintain a concise, non-repetitive accessibility description.
