...
**Learning:** In React Native, ephemeral notifications like Toasts may not be automatically announced by screen readers when they appear, causing visually impaired users to miss important context. Using `accessibilityRole="alert"` combined with `accessibilityLiveRegion="polite"` on the container ensures the message is read out without interrupting the user's current action.
**Action:** Always wrap ephemeral notification components (like Toasts or snackbars) with `accessibilityRole="alert"` and `accessibilityLiveRegion="polite"` to guarantee they are announced automatically.
