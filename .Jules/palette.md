...
## 2025-05-21 - Adding accessibility attributes to Progress Rings
**Learning:** In React Native, visual progress components (like circular progress rings) are not inherently understood by screen readers. Screen readers often read just the child text nodes, without context of the progress state.
**Action:** For visual progress components, wrap the component in a container with `accessible={true}`, `accessibilityRole="progressbar"`, `accessibilityValue={{ min, max, now }}`, and a localized `accessibilityLabel`. Ensure inner text nodes have `importantForAccessibility="no"` to prevent redundant reading by screen readers.
