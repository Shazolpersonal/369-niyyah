...
function NotificationHandler() {
    ...
    useEffect(() => {
        ...
        const subscription = Notifications.addNotificationResponseReceivedListener((response) => {
            ...
            if (data?.slot) {
                // Validate external notification payload data to prevent spoofing
                if (['morning', 'noon', 'night'].includes(data.slot as string)) {
                    // Record the hour to adapt future push times
                    recordNotificationInteraction(data.slot as any);
                } else {
                    logger.warn('Received invalid slot in notification payload data', { slot: data.slot });
                }
            }
