...
## 2025-06-10 - Unvalidated Push Notification Payloads
**Vulnerability:** The application was processing push notification payloads (`data?.slot` in `app/_layout.tsx`) without explicit validation, passing them directly to analytics and interaction recording logic as if they were safe.
**Learning:** In Expo/React Native, push notification payload data (e.g., `response.notification.request.content.data`) is untrusted external input and can be spoofed, leading to unexpected application behavior or poisoned analytics if passed directly into handlers.
**Prevention:** Explicitly validate all push notification payload data against an allowlist of expected shapes (e.g., specific TimeSlots) before processing them in application logic.
