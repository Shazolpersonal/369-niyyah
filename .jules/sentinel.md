...
## 2025-06-06 - Push Notification Payload Validation
**Vulnerability:** Untrusted external input (`response.notification.request.content.data`) from Expo push notifications was being used directly without validation to update analytics/logic state.
**Learning:** Push notification payloads can be modified, delayed, or crafted maliciously if an attacker compromises the notification delivery channel or exploits intents on Android. Always treat `.data` payloads as untrusted external input.
**Prevention:** Explicitly validate notification payloads (e.g., against an allowlist of expected TimeSlots) before processing them in internal application logic to prevent spoofing or unexpected downstream behavior.
