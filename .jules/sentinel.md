...
## 2025-05-19 - Untrusted Push Notification Payload Processing
**Vulnerability:** The application was processing push notification payload data (`data.slot`) without validation, treating it as trusted input and casting it directly to internal types (`TimeSlot`). This could allow spoofing or unexpected application behavior if malformed data were received.
**Learning:** In Expo/React Native, push notification payload data (`response.notification.request.content.data`) is untrusted external input. Even local notifications could potentially be spoofed or tampered with on rooted devices or via malicious apps with notification access.
**Prevention:** Always explicitly validate incoming notification payloads against an allowlist of expected values before processing them in application logic.
