...
## 2025-05-24 - Untrusted Push Notification Payload Validation
**Vulnerability:** The push notification interaction handler (`Notifications.addNotificationResponseReceivedListener`) in `app/_layout.tsx` accepted `data.slot` directly from the notification payload (`response.notification.request.content.data`) and passed it to `recordNotificationInteraction` without validating it against the allowed `TimeSlot` types (`'morning'`, `'noon'`, `'night'`).
**Learning:** Push notification payloads should be treated as untrusted external input. Failing to validate them before passing them to internal functions (especially those interacting with storage or analytics) can lead to unexpected state or logic bypasses if the payload is spoofed or malformed.
**Prevention:** Always explicitly validate incoming notification payload data against an expected allowlist or schema before processing it in application logic.
