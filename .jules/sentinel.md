...
**Vulnerability:** Untrusted external input from push notification payloads (`response.notification.request.content.data`) was being processed directly by the application logic without validation.
**Learning:** Any data coming from external sources, including push notification payloads handled by the OS, must be treated as untrusted and potentially spoofed.
**Prevention:** Always validate external inputs against an allowlist of expected types or values (e.g., verifying `data.slot` is a valid `TimeSlot` string) before passing them to internal processing functions.
