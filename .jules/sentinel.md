...
## 2025-02-22 - Unvalidated Push Notification Payload
**Vulnerability:** Push notification payload data was being passed directly to internal application logic without validation.
**Learning:** In Expo/React Native, push notification payload data (e.g., response.notification.request.content.data) is considered untrusted external input and can lead to unexpected behavior or spoofing if unvalidated.
**Prevention:** Always validate external payload data against an allowlist of expected values before processing it in application logic.
