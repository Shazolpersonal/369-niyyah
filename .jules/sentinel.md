...
## 2026-06-22 - Untrusted Push Notification Payload Data
**Vulnerability:** The push notification interaction handler blindly accepted and processed payload data (`response.notification.request.content.data.slot`) without validation, passing potentially spoofed or malicious input directly into application logic (`recordNotificationInteraction`).
**Learning:** Push notification payloads, even those originating from the OS, should be treated as untrusted external input. Without strict validation, an attacker or a malformed message could cause unexpected application state changes or logic failures.
**Prevention:** Explicitly validate all notification payload data against an allowlist of expected values (e.g., checking against valid `TimeSlot` values) before processing them in application logic.
