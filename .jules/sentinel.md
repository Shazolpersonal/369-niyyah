...
## 2025-02-21 - Unvalidated Push Notification Payload
**Vulnerability:** The notification interaction listener in `app/_layout.tsx` was reading `response.notification.request.content.data.slot` directly from the push payload and passing it to internal application logic (`recordNotificationInteraction`) without validating that the value matched the expected `TimeSlot` types.
**Learning:** Push notification payloads (`data` object) come from external OS APIs and should be treated as untrusted external input, even if the app scheduled the push itself. Malicious payloads or OS bugs could inject unexpected types or values, causing downstream logic failures or corruption of stored user state.
**Prevention:** Always validate push notification payload data explicitly (e.g., against an allowlist of expected keys/values) before casting types or using the data in application logic to prevent spoofing or unexpected behavior.
