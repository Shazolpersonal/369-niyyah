...
## 2025-06-04 - Fix Untrusted Push Notification Payload Processing
**Vulnerability:** The notification response handler in `app/_layout.tsx` was directly using the untrusted `data.slot` field from the push notification payload to record interactions, without validating it against the expected allowlist of slots.
**Learning:** External inputs, even those originating from push notification payloads sent by the app's own backend or local scheduling, should be treated as untrusted. They can be manipulated or contain unexpected data, which could lead to unexpected application state or crashes if passed directly into internal functions.
**Prevention:** Always validate external payloads (e.g., against an allowlist of expected values) before processing them in application logic.
