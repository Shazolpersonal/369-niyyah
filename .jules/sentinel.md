...
## 2025-06-17 - Unvalidated Notification Payload Input
**Vulnerability:** The application blindly accepted the `data.slot` value from incoming push notification payloads and passed it directly to internal logic (`recordNotificationInteraction`). Notification payloads are untrusted external inputs.
**Learning:** Even on mobile devices, data originating from push notifications (or deep links) can be spoofed or manipulated. Using this data without validation can lead to unexpected state changes, app crashes, or logic bypasses.
**Prevention:** Always explicitly validate incoming payloads (e.g., against an allowlist of expected values) before processing them in application logic.
