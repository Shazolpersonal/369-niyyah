...
## 2025-05-19 - Unvalidated Push Notification Payloads
**Vulnerability:** The application was directly parsing and using push notification payload data (`response.notification.request.content.data.slot`) without verifying its contents against expected values.
**Learning:** Push notification payloads, especially deep links and custom data, should be treated as untrusted external input. Failing to validate them can lead to unexpected state changes or application crashes if malformed data is processed by internal logic.
**Prevention:** Always validate external payload data against an explicit allowlist or expected schema before using it in application state or analytics functions.
