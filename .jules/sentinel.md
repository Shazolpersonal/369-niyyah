...
## 2025-05-19 - Untrusted Push Notification Payload Data
**Vulnerability:** Push notification payload data was being processed and passed directly into `recordNotificationInteraction` without validation.
**Learning:** In Expo/React Native, push notification payload data (e.g., `response.notification.request.content.data`) is considered untrusted external input and can be manipulated or spoofed.
**Prevention:** Explicitly validate these payloads (e.g., against an allowlist of expected types like TimeSlots) before processing them in application logic to prevent spoofing or unexpected behavior.
