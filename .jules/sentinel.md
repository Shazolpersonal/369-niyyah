...
## 2025-06-08 - Untrusted Push Notification Payload Processing
**Vulnerability:** Push notification payload data (`response.notification.request.content.data.slot`) was being read and passed directly into the application's core logic (`recordNotificationInteraction(data.slot as any)`) without any explicit validation.
**Learning:** Any data arriving via push notification payloads must be treated as untrusted external input. Without validation, an attacker or malfunctioning server could inject arbitrary strings, potentially causing unexpected behavior, analytics poisoning, or crashes when passed to internal state handlers expecting restricted literal types.
**Prevention:** Always explicitly validate untrusted payloads (e.g., against an allowlist of expected `TimeSlot`s) before casting or processing them in application logic.
