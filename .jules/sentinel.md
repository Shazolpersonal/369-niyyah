...
## 2025-02-21 - Fix Notification Payload Validation (Untrusted Input)
**Vulnerability:** External push notification payloads (e.g., `response.notification.request.content.data.slot`) were being used directly to trigger application analytics without any runtime type-checking or allow-listing.
**Learning:** Even though push notifications originate from controlled infrastructure, their payloads cross the application boundary as untrusted external input. Failing to validate this input opens up avenues for logic bypasses, spoofing, or passing unexpected data shapes that could crash background handlers.
**Prevention:** Always validate external payloads against an explicit allow-list (e.g., `['morning', 'noon', 'night']`) before using them in application logic, and gracefully reject or log invalid data.
