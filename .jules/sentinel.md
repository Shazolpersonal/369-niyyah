...
**Vulnerability:** Push notification payload data (`response.notification.request.content.data`) was being processed by application logic (`recordNotificationInteraction`) without prior validation, which could allow spoofed or unexpected payloads to pollute analytics or cause errors.
**Learning:** In Expo/React Native, data received from push notification payloads originates externally and must be treated as untrusted input.
**Prevention:** Always validate external payload data (e.g., using an allowlist like `['morning', 'noon', 'night']`) before using it in application logic.
