...
## 2025-05-19 - Validate Push Notification Payloads
**Vulnerability:** Untrusted external input from push notification payloads (`response.notification.request.content.data`) was being processed without validation.
**Learning:** Any data originating outside the app's direct control, including push notification payloads, must be treated as untrusted and validated against expected types and values to prevent spoofing or injection.
**Prevention:** Explicitly validate external payloads against an allowlist of expected values (e.g., `TimeSlot` values) before processing them in application logic.
