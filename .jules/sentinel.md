...
## 2026-09-05 - Missing validation for notification payload
**Vulnerability:** Untrusted external payload data `data?.slot` from push notifications was being used in `recordNotificationInteraction` without validation.
**Learning:** External push notification payloads must be treated as untrusted input. Directly parsing payload fields as enum types or internal states can lead to spoofing or unexpected errors.
**Prevention:** Explicitly validate push notification payloads against an allowlist of expected values before processing them in application logic to prevent spoofing or unexpected behavior.
