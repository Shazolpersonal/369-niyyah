...
## 2025-05-25 - Untrusted Push Notification Payload Data
**Vulnerability:** The application blindly trusted the `data.slot` property from the incoming push notification payload (`response.notification.request.content.data`) and cast it to `TimeSlot` without validation.
**Learning:** Push notification payloads are considered untrusted external input in Expo/React Native. Blindly casting and using them can lead to unexpected behavior or spoofing.
**Prevention:** Always explicitly validate incoming push notification payloads (e.g., using an allowlist) before processing them in application logic.
