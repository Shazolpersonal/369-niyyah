...
## 2025-02-20 - Optimize Keystroke Validation Memory Allocations
**Learning:** Using array spreads (`[...string]`) on text fields inside high-frequency functions like keystroke validation or highlighting creates heavy O(N) array allocations that run on every keypress. This can cause sluggishness or garbage collection pauses.
**Action:** Replace `[...string]` and `slice` operations with `startsWith()`, `substring()`, and manual `while` loops with `charCodeAt()` traversing, ensuring proper handling of Unicode surrogate pairs by manually skipping trailing surrogates.
