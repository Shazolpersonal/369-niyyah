...
## 2024-07-28 - Optimize String Indexing with Array Allocation
**Learning:** Hot paths validating inputs (e.g. `getValidationInfo` and `getHighlightSegments` operating on keystrokes) used O(N) array allocation via string spreading `[...str]` to handle Unicode safely. This incurs a massive performance hit.
**Action:** Replace `[...str]` array allocations with manual Unicode string traversal utilizing `charCodeAt()`, `codePointAt()`, and native operations like `startsWith` and `substring()`.
