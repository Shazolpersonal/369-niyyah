...
## 2024-06-28 - Avoid Repeated Function Calls Returning Primitive or Status in Hot Paths

**Learning:** The `isSlotActive` function was repeatedly evaluating the current slot via `getCurrentSlot()` internally (which instances `new Date()`) for every TaskCard render. Caching `getCurrentSlot()` once reduces multiple `new Date()` allocations per render frame.
**Action:** When conditionally evaluating UI states based on a single source of truth (like the current time slot), compute the base value once and use simple primitive equality checks (`currentSlot === 'morning'`) rather than calling status helper functions multiple times.
