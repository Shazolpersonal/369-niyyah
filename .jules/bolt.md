...
## 2024-07-02 - Caching Repeated Date Instantiations in Render Cycle
**Learning:** Calling `isSlotActive` multiple times for different slots during a single render cycle instantiated multiple `Date` objects, adding unnecessary garbage collection overhead and potential UI micro-stutters.
**Action:** Cache repeated expensive function calls that instantiate `Date` objects (like `getCurrentSlot()`) in local variables instead of calling them multiple times inline to reduce garbage collection overhead in React render cycles.
