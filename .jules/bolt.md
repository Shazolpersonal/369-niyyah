...
## 2024-06-27 - Avoid Array Spread Allocations for Unicode Strings
**Learning:** In hot loops processing text character-by-character (like keystroke validation logic in `textValidator.ts`), using `[...str]` to safely handle Unicode surrogate pairs (like emojis) introduces heavy `O(N)` array allocation overhead on every render cycle.
**Action:** Replace `[...str]` with zero-allocation `while` loops iterating over the string using `charCodeAt()`. For Unicode safety, explicitly check if the code unit falls within the high surrogate range (`0xD800` to `0xDBFF`) and increment the index appropriately.
