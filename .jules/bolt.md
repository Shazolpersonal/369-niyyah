...
## 2024-09-06 - Avoid O(N) Array Allocations via String Spread
**Learning:** Hot loops during typing validation (`getValidationInfo` and `getHighlightSegments`) were previously spreading strings into arrays (`[...str]`) to handle Unicode characters, causing memory allocation bottlenecks on every keystroke.
**Action:** When validating strings in hot paths, avoid O(N) array allocation. Use native string methods like `startsWith()`, `substring()`, and manual `charCodeAt()` loops for safe unicode processing.
