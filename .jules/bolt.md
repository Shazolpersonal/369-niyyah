...
## 2024-08-01 - Avoid String Spreading in Hot Paths
**Learning:** In hot loops validating keystrokes (like `getValidationInfo` and `getHighlightSegments`), spreading strings into arrays (`[...str]`) creates unnecessary O(N) array allocations, causing high CPU and garbage collection overhead.
**Action:** Use native string operations like `.charAt()`, `.length`, and `.substring()` directly on strings in performance-critical validation or highlighting loops to avoid unnecessary object allocations.
