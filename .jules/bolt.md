...
## 2024-07-04 - Caching Date-Instantiating Functions in Render Cycles
**Learning:** In `Dashboard.tsx` (`app/(tabs)/index.tsx`), calling `isSlotActive(slot)` for each of the three `TaskCard` components caused `getCurrentSlot()` to execute three times per render. Since `getCurrentSlot()` instantiates a `new Date()` internally, this created unnecessary garbage collection overhead due to multiple object allocations in a hot render path.
**Action:** To reduce garbage collection overhead in React render cycles, cache repeated expensive function calls that instantiate `Date` objects (like `getCurrentSlot()`) in local variables instead of calling them multiple times inline.
