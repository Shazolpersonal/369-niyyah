...
## 2026-06-28 - Cache Repeated Date Instantiations in Render Loops
**Learning:** The `isSlotActive` helper calls `getCurrentSlot()`, which instantiates a `new Date()` on every call. In the Dashboard view, checking the status of three distinct slots resulted in three separate `Date` object creations per render, causing unnecessary garbage collection overhead.
