...
## 2025-02-24 - Cache Expensive Function Calls in Render Loop
**Learning:** Repeatedly calling functions that instantiate `Date` objects inside React render loops (like checking `isSlotActive` multiple times for different items) causes unnecessary memory allocations and garbage collection overhead.
**Action:** Always cache the result of expensive helper functions (like `getCurrentSlot()`) in a local variable before passing the evaluated conditions down to components.
