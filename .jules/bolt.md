...
## 2024-07-05 - Caching Date Instantiations in Render Loops
**Learning:** To reduce garbage collection overhead in React render cycles, caching repeated expensive function calls that instantiate `Date` objects (like `getCurrentSlot()`) in local variables is more efficient than calling them multiple times inline (like `isSlotActive`).
**Action:** Always cache results of functions that instantiate `Date` objects if they are used multiple times in the same render cycle.
