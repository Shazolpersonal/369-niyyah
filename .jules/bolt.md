...
## 2024-05-26 - Prevent redundant Date instantiations in render blocks
**Learning:** In hot paths and React render loops, repeated calls to functions that internally instantiate new `Date()` objects (like `isSlotActive` which called `getCurrentSlot`) accumulate unnecessary garbage collection overhead, particularly in lists or clustered component groups.
**Action:** Always cache the result of expensive helper functions (e.g. `getCurrentSlot()`) in a local render variable when multiple sibling components need to evaluate state against the same underlying data, avoiding redundant object allocations.
