...
## 2024-04-19 - Memoizing JourneyProgressRing
**Learning:** `JourneyProgressRing` is a complex SVG animation component in `Dashboard` whose props (`currentDay`, `totalDays`) do not change during typical interaction within a day. Memoizing it prevents expensive recalculations and re-renders when other parts of the dashboard state change.
**Action:** Identify pure visual or complex components with static props in high-frequency re-rendering parent components and use React.memo to optimize them.
