...
## 2024-07-06 - Caching Repeated Expensive Date Function Calls
**Learning:** The `Dashboard` component was calling `isSlotActive(slot)` three times per render cycle, which in turn called `getCurrentSlot()`, instantiating three `Date` objects and creating unnecessary garbage collection overhead.
**Action:** To reduce GC overhead in React render cycles, always evaluate and cache expensive function calls that instantiate `Date` objects (like `getCurrentSlot()`) in local variables instead of calling them multiple times inline.
