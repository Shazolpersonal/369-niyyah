...
## 2025-02-18 - String Manipulation in Hot Paths
**Learning:** Using string spreading (`[...str]`) in hot loops like keystroke validation generates significant O(N) array allocation overhead and triggers frequent garbage collections.
**Action:** Replace `[...str]` array spreading with native string operations (`startsWith()`, `substring()`, and `charCodeAt()`) along with explicit low surrogate checks for safe Unicode parsing in hot paths.
