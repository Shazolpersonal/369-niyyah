...
## 2026-06-18 - Cache expensive Date allocations in render loop
**Learning:** Repeating function calls that instantiate `Date` objects in hot paths or render loops (like `getCurrentSlot()`) causes unnecessary garbage collection overhead.
**Action:** To reduce garbage collection overhead in React render cycles, cache repeated expensive function calls that instantiate `Date` objects in local variables instead of calling them multiple times inline.
